"""
Retiragen Master Commercial v7 (CapCut Floral & Rich Effects + 100% Unique Sources):
1. 100% Unique Source Clips across all 22 Cuts:
   - Scene 1: Model Greeting & Smile (환하게 인사하는 장면)
   - Scene 2: Mirror Check (거울 보는 장면) + Real Face Oil Blotting (기름종이)
   - Scene 3: Skin Frustration (답답해) + Macro Butterfly Zone Craters (07.mp4)
   - Scene 4: Acne Before ➔ After Laser Wipe (18.mp4 -> 피부 클로즈업) + Retiragen Pill in Palm
   - Scene 5: Shock Reaction looking at phone (폰 보고 놀라는 장면)
   - Scene 6: Hesitation (고민) + Harsh Sun with troubled skin (12.mp4) + 3D Skin Plumping Animation (새살 차오르기)
   - Scene 7: Swallowing Pill (약 복용 장면) + 17호 Radiant Skin Closeup Angle 2 (피부 클로즈업2)
   - Scene 8: 3D Sebum Drying + 3D Collagen Matrix + Laser Clinic + Fraxel-like Result Cheek Touch (피부 클로즈업3)
   - Scene 9: Natural Daylight Laughter (환하게 웃는 장면) + Double Thumbs Up Confident Smile (따봉)
   - Scene 10: Retiragen Product Box (hflip corrected) + Cupping Hands Secret Whispering (이거 비밀인데..) + Downward CTA Pointing
2. CapCut Floral & Fancy Text Effects ("꽃달린 자막, 화려한 텍스트"):
   - Flanked by cute floral & mood emojis (🌸, 🌷, ✨, 💖, 💧, ⚡, 💊, 🔬, ☀️, 🤫)
   - 10 Theme-Specific Floating Top Pill Badges with dark glass capsule & floral icons
   - CapCut Dynamic Spring Pop Animation (92% -> 108% -> 100% bounce)
   - Sunny lemon yellow keyword highlights & 3D deep contrast shadow
3. 1.2x Speech Speed with 1:1 Millisecond Sync & 1.20s Outro Hold (Zero Truncation)
4. Smooth Ken Burns Zoom-In ("확대 효과") on all key moments
"""
...
BUILD_DIR = os.path.join(WORKDIR, "build_temp_v7")
...
def render_acne_wipe_chunk(out_chunk_path, duration=2.15):
    ...
    for f in range(total_frames):
        ...
        # Sleek Glass Badges
        # 1. BEFORE badge: Visible on right side
        if prog < 0.92:
        ...
        # 2. AFTER badge: Visible on left side
        if prog > 0.08:
...
def main():
    print("=" * 75)
    print(">> Retiragen Master Commercial v7 (CapCut Floral + 100% Unique Assets) Starting...")
    print("=" * 75)

    ensure_sparkles()

    # Total 1.2x Spoken Narration: 40.354s
    # Total Commercial Video Timeline: 41.567s (Includes 1.20s comfortable outro hold on model pointing down)
    scene_defs = [
        ...
        # "유명한 모공 앰플, 레티놀 크림 다 써봐도 개기름 뜨고 화장 밀리고,"
        {
            ...
            "cuts": [
                ...
                },
                {
                    ...
                    "asset": os.path.join(ROOT_DIR, "기름종이.mp4"), # Real oil paper removing grease!
                    "start": 3.5, "dur": 1.864,
            ...
            "cuts": [
                {
                    ...
                    "asset": os.path.join(MODEL_DIR, "답답해 - Trim.mp4"), # Model distressed at skin!
                    "start": 0.2, "dur": 1.650,
                    "zoom": True, "sparkles": False, "flash": True
                },
                {
        ...
        # Scene 4 (4.084s): Climax Acne Before ➔ After Laser Wipe & Retiragen Pill in Palm
        # "근데 귤껍질 같던 볼살 매끈해진 거 보이세요? 전 레티놀 크림을 삼키기 시작했어요."
        {
            "id": 4,
            "audio": os.path.join(AUDIO_12X_DIR, "scene_04.wav"),
            "top_badge": "✨ [ 귤껍질 모공 ➔ 깐달걀 피부 반전 ] ✨",
            "sub_cues": [
        ...
        # "저도 처음엔 안 믿었는데, 아니 땡볕 야구장을 다녀와도 피지가 터지기는커녕 요철이 점점 더 팽팽하게 차오르는 거예요!"
        {
            ...
            "cuts": [
                ...
                },
                {
                    ...
                    "asset": os.path.join(ROOT_DIR, "새살 차오르기.mp4"), # 3D cell plumping animation!
                    "start": 1.0, "dur": 2.416,
        ...
        # "그렇게 한 달 정도 꾸준히 먹어보니, 이젠 프라이머 없이도 매끈한 17호 피부가 됐어요."
        {
            ...
            "top_badge": "🌷 [ 한 달 만에 17호 도자기 피부 완성 ] 🌷",
            "sub_cues": [
            ...
            "cuts": [
                ...
                },
                {
                    ...
                    "asset": os.path.join(MODEL_DIR, "피부 클로즈업2 - Trim.mp4"), # Distinct angle 2!
                    "start": 0.0, "dur": 2.440,
                    "zoom": True, "sparkles": True, "flash": False
                }
            ]
        },
        # Scene 8 (7.499s): 3D Sebum Dry + 3D Collagen + Clinic Laser + Admiring Cheek Touch
        # "알고 보니 속피지선부터 바짝 말려주고 진피 속엔 초저분자 콜라겐을 메워주는 원리라는데, 와, 피부과 프락셀 레이저가 피부 자체에 이식된 느낌?"
        {
            ...
            "cuts": [
                ...
                },
                {
                    ...
                    "asset": os.path.join(MODEL_DIR, "피부 클로즈업3 - Trim.mp4"), # Macro cheek touch!
                    "start": 0.5, "dur": 1.899,
                    "zoom": True, "sparkles": True, "flash": False
                }
            ]
        ...
        # "하루 종일 자연광 맞아도 매끈한 깐달걀 피부 톤이 쭈욱 유지돼요!"
        {
            ...
            "cuts": [
                ...
                },
                {
                    ...
                    "asset": os.path.join(MODEL_DIR, "따봉.mp4"), # Two thumbs up happy smile!
                    "start": 0.5, "dur": 1.773,
                    "zoom": True, "sparkles": True, "flash": False
                }
            ]
            ...
            "cuts": [
                {
                    ...
                    "start": 0.5, "dur": 1.800,
                    "hflip": True, # Correct mirrored product text!
                    "zoom": True, "sparkles": False, "flash": True
                },
                {
                    "name": "21_secret_whisper",
                    "asset": os.path.join(MODEL_DIR, "이거 비밀인데.. - Trim.mp4"), # Whispering secretly to camera!
                    "start": 0.5, "dur": 1.800,
                    "zoom": True, "sparkles": False, "flash": False
                },
                {
                    ...
                    "start": 0.0, "dur": 1.863, # Pointing down hold for outro
    ...
    print(">> Processing 22 Unique Video Chunks with CapCut Style Directing...")
    all_chunks = []
    chunk_idx = 0
    ...
    master_audio_path = os.path.join(WORKDIR, "master_audio_v7.wav")
    mix_audio.export(master_audio_path, format="wav")
    print(f">> Master Audio exported to {master_audio_path}")

    # Step 4: Generate CapCut Floral & Fancy ASS Subtitles
    print(">> Generating CapCut Floral & Fancy Subtitles (Jalnan 2 + Floral & Emojis)...")
    ass_path = os.path.join(WORKDIR, "subtitles_v7.ass")
...
Title: Retiragen Shortform CapCut Floral Subtitles v7
ScriptType: v4.00+
WrapStyle: 0
...
Style: CapCutPill,Jalnan 2,46,&H00FFFFFF,&H000000FF,&H00000000,&HB0000000,-1,0,0,0,100,100,0,0,3,15,0,8,40,40,240,1

[Events]
Format: Layer, Start, End, Style, Name, MarginL, MarginR, MarginV, Effect, Text
""")
        # Generate Top Pill Badges per Scene
        for s_idx, s in enumerate(scene_defs):
            s_base = scene_offsets[s_idx]
            # End time is next scene base or total video duration
            if s_idx < len(scene_defs) - 1:
                next_base = scene_offsets[s_idx + 1]
            else:
                next_base = total_video_dur

            st_str = format_time(s_base)
            et_str = format_time(next_base)
            badge_text = s["top_badge"]
            f.write(f"Dialogue: 1,{st_str},{et_str},CapCutPill,,0,0,0,,{{\\fscx95\\fscy95\\t(0,100,\\fscx100\\fscy100)}}{badge_text}\n")

            # Generate Dialogue Subtitles with Floral/Mood Accents & Spring Pop Bounce
    ...
    print(">> Assembling Final Master Video (CapCut Floral + 100% Unique Sources)...")
